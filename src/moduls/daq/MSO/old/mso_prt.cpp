...
bool TProtIn::mess( const string &ireqst, string &answer, const string &sender )
{
    ...
    string reqst = ireqst;
    bool isBuf = false;
    mess_info( nodePath().c_str(), _("MSO received <%d>bytes from <%u>!"), reqst.size(),node );

    AutoHD<TTpContr> t = SYS->daq().at().at("MSO");
    t.at().DataIn(reqst,sender);
    vector<string> lst;
...
//	if( modPrt->nAt(nls[i_n]).at().req(srcTr(),prt,node,req) ) break;
