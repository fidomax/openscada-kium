		...
		DA(prm), ID(id)
{
	...
	for (int i = 1; i <= 4; i++) {
		...
		mPrm->p_el.fldAdd(
				fld = new TFld(TSYS::strMess("save_params_%d", i).c_str(), TSYS::strMess(_("Save params %d"), i).c_str(), TFld::Boolean, TVal::DirWrite));
		fld->setReserve(TSYS::strMess("17:%d:0", i + ID * 4));
		mPrm->p_el.fldAdd(
				fld = new TFld(TSYS::strMess("calibrate_0_%d", i).c_str(), TSYS::strMess(_("Calibrate 0 %d"), i).c_str(), TFld::Boolean, TVal::DirWrite));
		fld->setReserve(TSYS::strMess("16:%d:5", i + ID * 4));
		mPrm->p_el.fldAdd(
				fld = new TFld(TSYS::strMess("calibrate_20_%d", i).c_str(), TSYS::strMess(_("Calibrate 20 %d"), i).c_str(), TFld::Boolean, TVal::DirWrite));
		fld->setReserve(TSYS::strMess("16:%d:6", i + ID * 4));
		mPrm->p_el.fldAdd(
				fld = new TFld(TSYS::strMess("stop_calibrate_%d", i).c_str(), TSYS::strMess(_("Stop calibration %d"), i).c_str(), TFld::Boolean,
						TVal::DirWrite));
		fld->setReserve(TSYS::strMess("16:%d:7", i + ID * 4));
		mPrm->p_el.fldAdd(
				fld = new TFld(TSYS::strMess("save_calibrate_%d", i).c_str(), TSYS::strMess(_("Save calibration %d"), i).c_str(), TFld::Boolean,
						TVal::DirWrite));
		...
		mPrm->p_el.fldAdd(
				fld = new TFld(TSYS::strMess("save_alarms_%d", i).c_str(), TSYS::strMess(_("Save alarms %d"), i).c_str(), TFld::Boolean, TVal::DirWrite));
