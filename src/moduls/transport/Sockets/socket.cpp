...
string TSocketIn::getStatus()
{
	...
	if (startStat())
		rez += TSYS::strMess(_("Connections %d, opened %d. Traffic in %s, out %s. Closed connections by limit %d."), connNumb,
				(protocol().empty() ? assTrs().size() : cl_id.size()), TSYS::cpct2str(trIn).c_str(), TSYS::cpct2str(trOut).c_str(), clsConnByLim);
...
void TSocketIn::load_()
{
	...
	try {
		...
		vl = prmNd.attr("MaxQueue");
		if (!vl.empty())
			setMaxQueue(atoi(vl.c_str()));
		vl = prmNd.attr("MaxClients");
		if (!vl.empty())
			setMaxFork(atoi(vl.c_str()));
		vl = prmNd.attr("BufLen");
		if (!vl.empty())
			setBufLen(atoi(vl.c_str()));
		vl = prmNd.attr("KeepAliveReqs");
		if (!vl.empty())
			setKeepAliveReqs(atoi(vl.c_str()));
		vl = prmNd.attr("KeepAliveTm");
		if (!vl.empty())
			setKeepAliveTm(atoi(vl.c_str()));
		vl = prmNd.attr("TaskPrior");
		if (!vl.empty())
			setTaskPrior(atoi(vl.c_str()));
	} catch (...) {
	}
}
...
//#if OSC_DEBUG >= 1
		if (mess_lev() == TMess::Debug)
			mess_debug(nodePath().c_str(), _("Socket <%s> created <%d>!"), s_type.c_str(), sock_fd);
//#endif
	} else
		throw TError(nodePath().c_str(), _("Socket type '%s' error!"), s_type.c_str());
	...
	if (type == SOCK_TCP || type == SOCK_UDP) {
		...
		if (type == SOCK_TCP) {
			...
			if (sptr != NULL)
				name_in.sin_port = sptr->s_port;
			else if (htons(atol(port.c_str())) > 0)
				name_in.sin_port = htons(atol(port.c_str()));
			else
				name_in.sin_port = 10001;
	...
	switch (type) {
		...
		case SOCK_UNIX: {
			...
			if (sId < 0)
				return -1;
			if (mess_lev() == TMess::Debug)
				mess_debug(nodePath().c_str(), _("Socket write message '%d'."), data.size());
			ssize_t wL = 1;
			unsigned wOff = 0;
			for (; wOff != data.size() && wL > 0; wOff += wL) {
				wL = write(sId, data.data() + wOff, data.size() - wOff);
				if (wL == 0) {
					mess_err(nodePath().c_str(), _("Write: reply for zero bytes."));
					break;
				} else if (wL < 0) {
					if (errno == EAGAIN) {
						tv.tv_sec = 1;
						tv.tv_usec = 0;		//!!!! Where the time take?
						FD_ZERO(&rw_fd);
						FD_SET(sId, &rw_fd);
						int kz = select(sId + 1, NULL, &rw_fd, NULL, &tv);
						if (kz > 0 && FD_ISSET(sId, &rw_fd)) {
							wL = 0;
							continue;
						}
					}
...
void *TSocketIn::Task(void *sock_in)
{
	...
	while (!sock->endrun) {
		...
		if (kz <= 0 || !FD_ISSET(sock->sock_fd, &rd_fd))
			continue;
...
void *TSocketIn::ClTask(void *s_inf)
{
	...
	do {
		...
		tv.tv_usec = STD_WAIT_DELAY * 1000;
		FD_ZERO(&rw_fd);
		FD_SET(s.cSock, &rw_fd);
		int kz = select(s.cSock + 1, &rw_fd, NULL, NULL, &tv);
		if (kz == 0 || (kz == -1 && errno == EINTR) || kz < 0 || !FD_ISSET(s.cSock, &rw_fd))
			continue;
		...
		if (answ.size()) {
			...
			for (unsigned wOff = 0; wOff != answ.size() && wL > 0; wOff += wL) {
				...
				if (wL == 0) {
					mess_err(s.s->nodePath().c_str(), _("Write: reply for zero bytes."));
					break;
				} else if (wL < 0) {
					if (errno == EAGAIN) {
						tv.tv_sec = 1;
						tv.tv_usec = 0;		//!!!! Where the time get?
						FD_ZERO(&rw_fd);
						FD_SET(s.cSock, &rw_fd);
						kz = select(s.cSock + 1, NULL, &rw_fd, NULL, &tv);
						if (kz > 0 && FD_ISSET(s.cSock, &rw_fd)) {
							wL = 0;
							continue;
						}
					}
...
void TSocketIn::messPut(int sock, string &request, string &answer, string sender, AutoHD<TProtocolIn> &prot_in)
{
	...
	try {
		if (prot_in.freeStat()) {
			...
			n_pr = id() + i2s(sock);
			if (!proto.at().openStat(n_pr))
				proto.at().open(n_pr, this, sender + "\n" + i2s(sock));
			prot_in = proto.at().at(n_pr);
		}
		if (prot_in.at().mess(request, answer))
			return;
		if (proto.freeStat())
			proto = AutoHD<TProtocol>(&prot_in.at().owner());
		...
		prot_in.free();
		if (!proto.freeStat() && proto.at().openStat(n_pr))
			proto.at().close(n_pr);
...
void TSocketIn::cntrCmdProc(XMLNode *opt)
{
	...
	if (opt->name() == "info") {
		...
		ctrMkNode("fld", opt, -1, "/prm/cfg/qLn", _("Queue length"), startStat() ? R_R_R_ : RWRWR_, "root", STR_ID, 2, "tp", "dec", "help",
				_("Used for TCP and UNIX sockets."));
		ctrMkNode("fld", opt, -1, "/prm/cfg/clMax", _("Clients maximum"),
		RWRWR_, "root", STR_ID, 2, "tp", "dec", "help", _("Used for TCP and UNIX sockets."));
		ctrMkNode("fld", opt, -1, "/prm/cfg/bfLn", _("Input buffer (kbyte)"), startStat() ? R_R_R_ : RWRWR_, "root", STR_ID, 1, "tp", "dec");
		ctrMkNode("fld", opt, -1, "/prm/cfg/taskPrior", _("Priority"), startStat() ? R_R_R_ : RWRWR_, "root", STR_ID, 4, "tp", "dec", "min", "-1", "max", "99",
				"help", TMess::labTaskPrior());
		ctrMkNode("fld", opt, -1, "/prm/cfg/keepAliveReqs", _("Keep alive requests"), RWRWR_, "root", STR_ID, 2, "tp", "dec", "help",
				_("Close the connection after specified requests.\nZero value for disable (not close ever)."));
		ctrMkNode("fld", opt, -1, "/prm/cfg/keepAliveTm", _("Keep alive timeout (s)"), RWRWR_, "root", STR_ID, 2, "tp", "dec", "help",
				_("Close the connection after no requests at specified timeout.\nZero value for disable (not close ever)."));
		return;
	}
	...
	if (a_path == "/prm/cfg/qLn") {
		if (ctrChkNode(opt, "get", RWRWR_, "root", STR_ID, SEC_RD))
			opt->setText(i2s(maxQueue()));
		if (ctrChkNode(opt, "set", RWRWR_, "root", STR_ID, SEC_WR))
			setMaxQueue(atoi(opt->text().c_str()));
	} else if (a_path == "/prm/cfg/clMax") {
		if (ctrChkNode(opt, "get", RWRWR_, "root", STR_ID, SEC_RD))
			opt->setText(i2s(maxFork()));
		if (ctrChkNode(opt, "set", RWRWR_, "root", STR_ID, SEC_WR))
			setMaxFork(atoi(opt->text().c_str()));
	} else if (a_path == "/prm/cfg/bfLn") {
		if (ctrChkNode(opt, "get", RWRWR_, "root", STR_ID, SEC_RD))
			opt->setText(i2s(bufLen()));
		if (ctrChkNode(opt, "set", RWRWR_, "root", STR_ID, SEC_WR))
			setBufLen(atoi(opt->text().c_str()));
	} else if (a_path == "/prm/cfg/keepAliveReqs") {
		if (ctrChkNode(opt, "get", RWRWR_, "root", STR_ID, SEC_RD))
			opt->setText(i2s(keepAliveReqs()));
		if (ctrChkNode(opt, "set", RWRWR_, "root", STR_ID, SEC_WR))
			setKeepAliveReqs(atoi(opt->text().c_str()));
	} else if (a_path == "/prm/cfg/keepAliveTm") {
		if (ctrChkNode(opt, "get", RWRWR_, "root", STR_ID, SEC_RD))
			opt->setText(i2s(keepAliveTm()));
		if (ctrChkNode(opt, "set", RWRWR_, "root", STR_ID, SEC_WR))
			setKeepAliveTm(atoi(opt->text().c_str()));
	} else if (a_path == "/prm/cfg/taskPrior") {
		if (ctrChkNode(opt, "get", RWRWR_, "root", STR_ID, SEC_RD))
			opt->setText(i2s(taskPrior()));
		if (ctrChkNode(opt, "set", RWRWR_, "root", STR_ID, SEC_WR))
			setTaskPrior(atoi(opt->text().c_str()));
	} else
		TTransportIn::cntrCmdProc(opt);
}
...
void TSocketOut::setTimings(const string &vl)
{
	...
	mTimings =
			mTmRep ? TSYS::strMess("%g:%g:%g", (1e-3 * mTmCon), (1e-3 * mTmNext), (1e-3 * mTmRep)) : TSYS::strMess("%g:%g", (1e-3 * mTmCon), (1e-3 * mTmNext));
	modif();
}
...
void TSocketOut::start(int itmCon)
{
	...
	if (s_type == S_NM_SOCK)
		...
		throw TError(nodePath().c_str(), _("Type socket '%s' error!"), s_type.c_str());
	...
	if (type == SOCK_FORCE) {
		...
		if (res == -1 && errno == EINPROGRESS) {
			...
			fd_set fdset;
			tv.tv_sec = itmCon / 1000;
			tv.tv_usec = 1000 * (itmCon % 1000);
			FD_ZERO(&fdset);
			FD_SET(sock_fd, &fdset);
			if ((res = select(sock_fd + 1, NULL, &fdset, NULL, &tv)) > 0 && !getsockopt(sock_fd, SOL_SOCKET, SO_ERROR, &res, &slen) && !res)
				res = 0;
			else
				res = -1;
		}
...
int TSocketOut::messIO(const char *obuf, int len_ob, char *ibuf, int len_ib, int time, bool noRes)
{
	...
	if (!run_st)
		throw TError(nodePath().c_str(), _("Transport is not started!"));

	repeate: if (reqTry++ >= 2) {
		mLstReqTm = TSYS::curTime();
		throw TError(nodePath().c_str(), _("Request error: %s"), err.c_str());
	}
	//Write request
	writeReq = false;
	if (obuf != NULL && len_ob > 0) {
		...
		for (int wOff = 0; wOff != len_ob; wOff += kz) {
			...
			if (kz <= 0) {
				if (errno == EAGAIN) {
					tv.tv_sec = (time / 2) / 1000;
					tv.tv_usec = 1000 * ((time / 2) % 1000);
					FD_ZERO(&rw_fd);
					FD_SET(sock_fd, &rw_fd);
					kz = select(sock_fd + 1, NULL, &rw_fd, NULL, &tv);
					if (kz > 0 && FD_ISSET(sock_fd, &rw_fd)) {
						kz = 0;
						continue;
					}
				}
...
void TSocketOut::cntrCmdProc(XMLNode *opt)
{
	...
	if (opt->name() == "info") {
		...
		ctrRemoveNode(opt, "/prm/cfg/A_PRMS");
		ctrMkNode("fld", opt, -1, "/prm/cfg/ADDR", EVAL_STR, RWRWR_, "root",
		STR_ID, 1, "help", _("Socket's output transport has address format:\n"
				...
				"    name - UNIX-socket's file name."));
		ctrMkNode("fld", opt, -1, "/prm/cfg/TMS", _("Timings"), RWRWR_, "root",
		STR_ID, 2, "tp", "str", "help", _("Connection timings in format: \"conn:next[:rep]\". Where:\n"
